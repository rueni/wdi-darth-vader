...
function sum(numbers, twiceOrHalf) {
  if (!Array.isArray(numbers)) {
    throw new Error('The first argument of "sum()" must be an array!');
  }

  if (twiceOrHalf === undefined) {
    return numbers.reduce(function(prev, current){ return prev + current });
  } else if (twiceOrHalf) {
    return numbers.reduce(function(prev, current){ return prev + current }) * 2;
  } else if (!twiceOrHalf) {
    return numbers.reduce(function(prev, current){ return prev + current }) / 2;
  }
...
function max() {
  return Array.prototype.slice.call(arguments).sort(function(a, b){ return b - a })[0];
}
